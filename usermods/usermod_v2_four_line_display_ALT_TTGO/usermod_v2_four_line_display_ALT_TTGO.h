...
// DAA //      lineHeight = u8x8->getRows() > 4 ? 2 : 1;
      lineHeight = 1;
      DEBUG_PRINTLN(F("Starting display."));
      setFlipMode(flip);
      setVcomh(contrastFix);
     //daa// setContrast(contrast); //Contrast setup will help to preserve OLED lifetime. In case OLED need to be brighter increase number up to 255
      setPowerSave(0);
  ...
  // DAA //    u8x8->setFlipMode(mode);
    ...
    // DAA //     u8x8->setFont(u8x8_font_chroma48medium8_r);
      ...
      if (!ignoreLH && lineHeight==2)   // DAA // u8x8->draw1x2String(col, row, string)
          ...
          tft.setCursor(col*16, row*25);
          tft.print(string);
          //tft.drawString(string,col*16,row*16,3);      
            } 
      else                              // DAA // u8x8->drawString(col, row, string)
        {
          ...
          tft.setCursor(col*16, row*25);
          tft.print(string);
          //ft.drawString(string,col*16,row*25,2);
       } 
    }
    ...
    // DAA //     u8x8->setFont(u8x8_font_chroma48medium8_r);
     ...
     // DAA //    u8x8->draw2x2String(col, row, string);
          ...
          tft.setCursor(col*16, row*25);
          tft.print(string);
          //tft.drawString(string,col*16,row*25,3);    
        } 

    void drawGlyph(uint8_t col, uint8_t row, char glyph,  const uint8_t *font, const bool ignoreLH=false) {
      if (type == NONE || !enabled) return;
    ...
    void draw2x2Glyph(uint8_t col, uint8_t row, char glyph, const uint8_t *font) {
      if (type == NONE || !enabled) return;
 	       tft.setTextFont(  4);
      if ( lineHeight==2)   // DAA //  u8x8->draw1x2Glyph(col, row, glyph)
         tft.drawChar(col,row,glyph);
      else 
         tft.drawChar(col,row,glyph);		  // DAA // u8x8->drawGlyph(col, row, glyph)
      ;
...
//daa//          if (type != NONE) delete u8x8;
 ...
 //daa//         u8x8->setBusClock(ioFrequency); // can be used for SPI too
          setVcomh(contrastFix);
